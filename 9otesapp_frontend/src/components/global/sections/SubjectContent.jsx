...
export default function SubjectContent() {
    ...
    useEffect(() => {
        const fetchSubject = async () => {
            try {
                const response = await fetch(`${baseUrl}/api/sharedSubject/getSubject/${subId}`);
    ...
    return (
        <div className="mt-3 w-full bg-gradient-to-br from-blue-100 to-indigo-200 p-6">
            <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-indigo-700 to-blue-500 p-6 text-white text-2xl md:text-3xl font-bold tracking-wide text-center">
                        ...
                        subject.units.map((unit) => (
                            <div key={unit.id} className="bg-gray-100 p-4 md:p-5 rounded-lg shadow-md">
                                ...
                                {unit.topics.length > 0 ? (
                                        ...
                                        <div key={topic.id} className="bg-white p-3 md:p-4 rounded-lg shadow-md mb-4">
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                <div className="w-full md:w-1/3 bg-gray-50 p-3 md:p-4 rounded-lg shadow">
                                                    <h3 className="text-md md:text-lg font-semibold text-gray-800">{topic.topicName}</h3>
                                                </div>
                                                <div className="w-full md:w-2/3 bg-gray-50 p-3 md:p-4 rounded-lg shadow">
                                                    <p className="text-gray-600 text-sm md:text-base">{topic.topicDescription}</p>
                                                </div>
                                                {/* Buttons (Wrapped for mobile responsiveness) */}
                                            ...
                                            {visibleFileTable === topic.id && topic.fileUrls.length > 0 && (
                                                <div className="overflow-x-auto max-w-full mt-4">
                                                    <table className="w-full text-xs md:text-sm text-gray-700 border border-gray-300 rounded-lg overflow-hidden">
                                                        <thead className="text-xs md:text-sm uppercase bg-gray-200">
                                                        <tr>
                                                            <th className="px-4 md:px-6 py-3">S No.</th>
                                                            <th className="px-4 md:px-6 py-3">File Name</th>
                                                            <th className="px-4 md:px-6 py-3">Date</th>
                                                            <th className="px-4 md:px-6 py-3">Time</th>
                                                            <th className="px-4 md:px-6 py-3">Comment</th>
                                                            <th className="px-4 md:px-6 py-3">Preview</th>
                                                        </tr>
                                                        </thead>
                                                        <tbody>
                                                        {topic.fileUrls.slice().reverse().map((fileUrl, index) => (
                                                            <tr key={fileUrl.id} className="border-b even:bg-gray-100">
                                                                ...
                                                                <td className="px-4 md:px-6 py-4">{fileUrl.fileName}</td>
                                                                <td className="px-4 md:px-6 py-4">{fileUrl.createdOn}</td>
                                                                <td className="px-4 md:px-6 py-4">{fileUrl.createdAt}</td>
                                                                <td className="px-4 md:px-6 py-4">{fileUrl.comment}</td>
                                    ...
                                    <p className="text-gray-500 text-center text-sm md:text-base">No topics available.</p>
                                )}
                            </div>
                        ))
                    ) : (
                        <p className="text-gray-500 text-center text-sm md:text-base">No units available.</p>
                    )}
                </div>
            </div>
        </div>
